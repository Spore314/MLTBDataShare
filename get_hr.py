...
def user_input(cal_done_list): 
    ...
    error_message = f"Invalid input: Expecting a positive integer N <= 999." 
...
def download_files(fs, fb, cal_done_list, N):  
    ...
    #for i, struct_id in enumerate(selected_struct_ids, 1):
        ...
        # Download files: POSCAR-KPT_SCF-INPUT-Input
        out = fb.get_last_version(struct_id=struct_id)
        file_path = os.path.join(folder_name, 'data-POS-KPT-download')
        with open(file_path, 'wb') as f:
            f.write(out.read())
        extract_files(folder_name, 'data-POS-KPT-download')

        # Remove tmp files: data-STRU-HR-download / data-POS-KPT-download
        os.remove(os.path.join(folder_name, 'data-STRU-HR-download'))
        os.remove(os.path.join(folder_name, 'data-POS-KPT-download'))
